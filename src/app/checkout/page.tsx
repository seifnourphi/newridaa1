...
export default function CheckoutPage() {
  ...
  const validatePostalCode = (): boolean => {
    ...
    if (postalCode.length !== 5) {
      setErrors(prev => ({ ...prev, postalCode: language === 'ar' 
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setErrors(prev => ({ ...prev, paymentProof: language === 'ar' 
  ...
  const handleValidateAndProceedToShipping = async () => {
    ...
    try {
      ...
      if (!validateResponse.ok) {
        console.error('Stock validation error:', validateResponse.status, validateData);
        showAlert(
  ...
  const handleUploadPaymentProof = async () => {
    ...
    try {
      ...
      if (!token && user) {
        ...
        console.warn('[Upload Payment Proof] No token found, redirecting to login');
        router.push(`/auth/login?redirect=/checkout`);
        return;
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          console.warn('[Upload Payment Proof] 401 Unauthorized - token may be invalid or expired');
          setErrors(prev => ({ ...prev, paymentProof: language === 'ar' 
      ...
      setErrors(prev => ({ ...prev, paymentProof: error.message || (language === 'ar' 
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-[#9333EA] mx-auto mb-4"></div>
            ...
            <div key={item.step} className="flex items-center">
              <div className="text-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold ${
                  item.step === currentStep ? 'bg-[#9333EA] text-white' : 'bg-gray-200 text-gray-600'
                }`}>
                  {item.step}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      maxLength={5}
                      inputMode="numeric"
                      className={`w-full px-4 py-3 border rounded-md focus:ring-2 focus:ring-[#9333EA] ${
                        errors.postalCode ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'
                      }`}
                    />
                    {errors.postalCode && (
